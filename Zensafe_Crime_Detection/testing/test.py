...
from utils.eval_utils import cal_auc,cal_score_gap,cal_false_alarm, cal_false_neg
...
def load_model_dataset(args):
    ...
    model=getattr(models, 'I3D_SGA_STD')(dropout_rate=args.dropout_rate,expand_k=args.expand_k,
    ...
    if args.MODEL.split('_')[-1]=='C3D':
        if args.MODEL=='UCF_C3D':
            ...
            dataset=Test_Dataset_SHT_C3D(_C.SHT_TEST_H5_PATH,_C.SHT_TEST_TXT_PATH,_C.SHT_TEST_MASK_DIR,
                                         args.segment_len,args.ten_cr]TSop)
        ...
        if args.MODEL=='UCF_I3D':
            dataset=Test_Dataset_I3D(_C.TEST_H5_PATH,_C.TESTING_TXT_PATH,args.segment_len,False)
...
def eval(total_scores,total_labels,normal_scores):
    ...
    fn = cal_false_neg(total_scores, total_labels)
    print('{}: AUC {:.2f}%, FAR {:.2f}%, GAP {:.2f}%, FN {:.2f}%'.format(args.MODEL,auc*100,far*100,gap*100, fn*100))
